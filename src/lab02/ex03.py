def format_record(rec: tuple[str, str, float]) -> str:
    ...
    if len(rec) != 3:  # в кортеже ровно 3 элемента
        raise ValueError
    if (
        type(rec[2]) is not float
    ):  # Проверяем, что GPA - это число с плавающей точкой (float)
        raise TypeError
    ...
    name_parts = (
        rec[0].strip().split()
    )  # убираем лишние пробелы в начале и конце, разбиваем строку на части по пробелам
    if len(name_parts) == 3:
        m1, m2, m3 = name_parts
