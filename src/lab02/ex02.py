def transpose(mat: list[list[float | int]]) -> list[list[float | int]]:
    ...
    row_len = len(mat[0])
    if any(
        len(row) != row_len for row in mat
...
def col_sums(mat: list[list[float | int]]) -> list[float]:
    ...
    return [
        sum(mat[i][j] for i in range(len(mat))) for j in range(row_len)
    ]  # j идёт по столбцам для каждого столбца j суммируем элементы всех строк i
